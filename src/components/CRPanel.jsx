...
export default function CRPanel({ node, isActive, isExpanded, onToggle }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-6 py-4">
        <div>
          <p className="font-heading text-xs tracking-[0.22em] text-accent uppercase">
            CR — PAYMENT INITIATION TRANSACTION
          </p>
              ...
              <div className="grid grid-cols-1 gap-1 sm:grid-cols-2">
                {node?.fields?.map((field) => (
                  <div key={field.name} className="flex items-baseline gap-2">
                    <span className="font-body text-xs text-accent/70 whitespace-nowrap">
                      {field.name}
                    </span>
